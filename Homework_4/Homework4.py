...
# join filtered to OG tumor and nromal files

# Second section - VCF files
# Read VCF files properly by setting header manually
tumor_vcf = pd.read_csv("/Users/yeseniajuarez/Documents/genomic_analysis/Homework_4/gen_coords_85db7982.vcf", 
                        sep='\t', skiprows=8)
normal_vcf = pd.read_csv("/Users/yeseniajuarez/Documents/genomic_analysis/Homework_4/gen_coords_f9fb2794.vcf", 
                         sep='\t', skiprows=8)

# Set proper column names for VCF format
tumor_vcf.columns = ['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO']
normal_vcf.columns = ['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO']

print("Columns:", tumor_vcf.columns.tolist())

# Now filter using the correct ID column
...
print(f"Tumor VCF: {tumor_vcf.shape} -> {tumor_vcf_filtered.shape}")
print(f"Normal VCF: {normal_vcf.shape} -> {normal_vcf_filtered.shape}")

# Read FATHMM files
fathmm_tumor = pd.read_csv("Homework_4/tumor_fathmm.txt", sep='\t')
fathmm_normal = pd.read_csv("Homework_4/normal_vcf.txt", sep='\t')

# CONCATENATE ALL AT THE END - Reset all indexes first:
